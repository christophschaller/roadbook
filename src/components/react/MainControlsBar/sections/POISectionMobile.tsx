...
function ResourceContainer({ resource }: { resource: PointOfInterest }) {
  if (!resource.color) {
    return null;
  }
  ...
  return (
    <div className="space-y-2">
      <div className="w-full flex items-center space-x-2 p-2 rounded-md hover:bg-white/50 transition-colors">
        ...
        >
          {resource.icon && (
            <img src={resource.icon} className="w-5 h-5" alt="Icon" />
          )}
        </div>
...
export function POISectionMobile() {
  const { pois: resources } = useStore(poiStore);
  ...
  const chevronLeftButton = (
    <button
      onClick={() => {
        ...
        handleResourceChange(String(resources[activeResourceIndex - 1].id));
      }}
